...
public class PanelUtil {
    ...
    public static void show(String key, boolean isCenter) {
        Main.primaryStage.setScene(new Scene(new AnchorPane()));
        Scene scene = getScene(key);
        setSceneStage(key, isCenter);
        Main.primaryStage.setScene(scene);
        if (!Main.primaryStage.isShowing()) {
            Main.primaryStage.show();
        }
    ...
    private static void setSceneStage(String key) {
        sceneStageMap.get(key).transfer(Main.primaryStage, true);
    }

    private static void setSceneStage(String key, boolean isCenter) {
        sceneStageMap.get(key).transfer(Main.primaryStage, isCenter);
    }
    ...
    public static void setWindowsInCenter() {
        Rectangle2D rtl = adapetScreen(Main.primaryStage).getVisualBounds();
        Main.primaryStage.setX(rtl.getMinX() + Math.abs((rtl.getWidth() - Main.primaryStage.getWidth()) / 2));
        Main.primaryStage.setY(rtl.getMinY() + Math.abs((rtl.getHeight() - Main.primaryStage.getHeight()) / 2));
    }

    private static Screen adapetScreen(Stage stage) {
        for (Screen screen : Screen.getScreens()) {
            Rectangle2D rtl = screen.getVisualBounds();
            if (stage.getX() >= rtl.getMinX() && stage.getX() <= rtl.getMaxX()
                && stage.getY() >= rtl.getMinY() && stage.getY() <= rtl.getMaxY()) {
                return screen;
            }
        }
